def get_prompt(method, query, documents_block):
    ...
    4. Stop or continue?
    Continue. The given information is too general to answer the query’s specific biological and evolutionary questions. Additional retrieval is necessary to obtain mechanistic and empirical evidence.
    </reason>
    ...
    4. Stop or continue?
    Stop. The existing documents are sufficient to answer the query, so no further retrieval is needed. No <summary> should be produced.
    </reason>
    <satisfy> yes </satisfy>
